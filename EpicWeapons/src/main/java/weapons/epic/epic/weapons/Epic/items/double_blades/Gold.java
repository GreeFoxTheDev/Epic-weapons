...
public class Gold extends JavaPlugin {
    public static ItemStack goldDoubleBlade;

    public static void init() {
        createGoldDoubleBlade();
    }

    private static void createGoldDoubleBlade() {
        ItemStack item = new ItemStack(Material.GOLDEN_AXE, 1);
        ItemMeta im = item.getItemMeta();
        im.setDisplayName(ChatColor.WHITE + "Golden Double Blade");
        im.setLocalizedName("gold_dblade");
        ...
        item.setItemMeta(im);
        goldDoubleBlade = item;
        ShapedRecipe sr = new ShapedRecipe(NamespacedKey.minecraft("gold_dblade"), item);
        sr.shape("  X", " Y ", "X  ");
